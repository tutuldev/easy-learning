...
<div class="max-w-2xl mx-auto mt-20 p-6 bg-white rounded-lg shadow-md border border-gray-100">
  ...
  <h2 class="text-3xl font-bold text-gray-800 mb-6 flex items-center gap-2">
    <span class="material-symbols-outlined text-blue-500">topic</span>
    Topic Details
  </h2>

  <!-- Topic ID -->
  <div class="mb-4">
    <p class="text-sm text-gray-500">Topic ID</p>
    <p class="text-lg font-medium text-gray-800">{{ $topic->id }}</p>
  </div>

  <!-- Topic Name -->
  <div class="mb-4">
    <p class="text-sm text-gray-500">Name</p>
    <p class="text-lg font-medium text-gray-800">{{ $topic->name }}</p>
  </div>

  <!-- Topic Description -->
  <div class="mb-6">
    <p class="text-sm text-gray-500">Description</p>
    <p class="text-base text-gray-700">{{ strip_tags($topic->description) }}</p>
  </div>

  <!-- Show All Posts Under This Topic -->
  <div class="mb-6">
    <a href="{{ route('admin.posts.topic', $topic->name) }}">
      <div class="bg-gray-50 hover:bg-gray-100 transition rounded-md py-4 px-5 border border-gray-200 flex justify-between items-center">
        <span class="text-sm text-gray-700">View all posts under <strong>{{ $topic->name }}</strong></span>
        <span class="text-xs bg-indigo-600 text-white px-2 py-1 rounded-full">Posts: {{ $topic->posts_count }}</span>
      </div>
    </a>
  </div>
