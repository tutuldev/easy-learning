@extends('backend.app')
@section('title', 'Create Structer')

@section('content')
<h2 class="mt-16 text-2xl font-semibold">Add Structer</h2>
...
<form action="{{ route('structers.store') }}" method="POST" class="max-w-sm bg-white p-6 rounded-lg shadow">
    @csrf

    <div class="mb-4">
        <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Structer Name</label>
        <input type="text" id="name" name="name" placeholder="Enter structer name"
