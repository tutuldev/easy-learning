...
<a href="{{ route('structers.create') }}" class="text-white px-2 py-1 text-sm rounded-md bg-green-600 hover:bg-green-700 my-2 inline-block">
    Add<span class="material-symbols-outlined align-middle text-sm mx-2">add</span>
</a>
    ...
    <table class="min-w-full table-auto border text-center border-gray-200">
        ...
        <tbody>
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-2 border">{{ $structer->id }}</td>
                <td class="px-4 py-2 border">{{ $structer->name }}</td>
                <td class="flex space-x-2 justify-center py-2">
                    <a href="{{ route('structers.show',  $structer->slug) }}" class="text-white px-2 py-1 rounded-md bg-indigo-600 hover:bg-indigo-700 text-sm">Show</a>
                    <a href="{{ route('structers.edit', $structer->slug) }}" class="text-white px-2 py-1 rounded-md bg-sky-600 hover:bg-sky-700 text-sm">Edit</a>
