  ...
  <div class="mb-6">
    <a href="{{ route('admin.posts.category', $category->name) }}" class="block">
      <div class="bg-gray-50 hover:bg-gray-100 transition rounded-md py-4 px-5 border border-gray-200 flex justify-between items-center">
        <span class="text-sm text-gray-700">View all posts under <strong>{{ $category->name }}</strong></span>
        <span class="text-xs bg-blue-600 text-white px-2 py-1 rounded-full">Posts: {{ $category->posts_count }}</span>
      </div>
    </a>
  </div>
