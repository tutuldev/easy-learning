@extends('backend.app')
@section('content')
<h2 class="text-2xl font-semibold ">Add Framework</h2>
...
<form action="{{ route('frameworks.store') }}" method="POST" class="max-w-sm  bg-white p-6 rounded-lg shadow">
    @csrf

    <div class="mb-4">
        <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Framework Name</label>
        <input type="text" id="name" name="name" placeholder="Enter framework name"
